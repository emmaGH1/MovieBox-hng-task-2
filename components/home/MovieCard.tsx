...
const MovieCard = ({ id, posterPath, releaseDate, title, voteAverage, popularity }: Props) => {
  ...
  return (
    <div data-testid="movie-card" className="flex flex-col mb-10 shadow-md" >
       ...
       <Link data-testid="movie-release-date" href={`/movies/${id}`} className="text-gray-400 px-2 mb-2 mt-2">USA, {year}</Link>
       <Link data-testid="movie-title" href={`/movies/${id}`} className="text-gray-900 text-lg font-bold px-2 mb-1">{title}</Link>
       <Link href={`/movies/${id}`} className="flex text-gray-400 justify-between px-2">
            ...
            <div className="flex items-center mb-3">
              ...
              <span> {popularity > 100 ? 100 : popularity.toFixed(0)}% </span>
            </div>
          </Link>
    </div>
  );
